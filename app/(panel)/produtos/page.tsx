    ...
    <div className="space-y-5">
      <div className="flex flex-wrap justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">As variações importadas da Shopee aparecem aqui. Informe gramas e tempo de impressão para calcular o custo real.</p>
        </div>
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={add} className="grid gap-3 md:grid-cols-4 xl:grid-cols-8">
            <div>
              <Label>Modo</Label>
              <select className="h-10 w-full rounded-md border bg-background px-3" value={mode} onChange={(e) => setMode(e.target.value as "new" | "existing")}>
                <option value="new">Novo produto</option>
                <option value="existing">Produto existente</option>
              </select>
            </div>
            {mode === "new" ? (
              <div><Label>Produto</Label><Input required value={name} onChange={(e) => setName(e.target.value)} /></div>
            ) : (
              <div>
                <Label>Produto</Label>
                <select required className="h-10 w-full rounded-md border bg-background px-3" value={productId} onChange={(e) => setProductId(e.target.value)}>
            ...
            {mode === "new" && customColumns.map((c) => (
              <div key={c.id}>
                <Label>{c.label}</Label>
                <Input type={c.data_type === "date" ? "date" : c.data_type === "number" ? "number" : "text"} value={custom[c.key] ?? ""} onChange={(e) => setCustom({ ...custom, [c.key]: e.target.value })} />
              </div>
            ))}
      ...
      <Card>
        ...
        <CardContent>
          {ranking.length === 0 ? (
            <p className="text-sm text-muted-foreground">Sem vendas pagas no mês.</p>
          ) : (
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
              {ranking.slice(0, 10).map((r, i) => (
                <div key={r.product_id} className="rounded-lg border p-3">
                  ...
                  <div className="text-sm">{r.quantity} un. · {formatBRL(r.revenue)}</div>
                </div>
              ))}
            </div>
          )}
      ...
      <Table>
        <THead>
          <TR>
            ...
            <TH>Variação</TH>
            <TH>SKU</TH>
            <TH>Consumo e custo / unidade</TH>
            <TH>Qtd mês</TH>
            <TH>Faturamento mês</TH>
            <TH>Sugestões 30/60/90d</TH>
            <TH>Estoque manual</TH>
            {customColumns.map((c) => <TH key={c.id}>{c.label}</TH>)}
          </TR>
        </THead>
        <TBody>
          {data.flatMap((p) => p.product_variants.map((v: any) => (
            <VariantRow
              key={v.id}
              p={p}
              v={v}
...
function VariantRow({ p, v, fees, st, s30, s60, s90, customColumns, onSaved }: any) {
  ...
  const cost = calculateProductionUnit({
    filamentPricePerKg: fees.filament_price_per_kg ?? "0",
    filamentGrams: filamentGrams || "0",
    energyPricePerKwh: fees.energy_price_per_kwh ?? "0",
    printTimeHours: printTimeHours || "0",
    printerPowerWatts: effectivePower || "0",
    packagingCost: effectivePackaging || "0",
  });
  ...
  async function save() {
    ...
    const { error } = await s.rpc("update_variant_settings_transaction", {
      ...
      p_packaging: pack === "" ? null : normalizeDecimal(pack),
  ...
  return (
    <TR>
      <TD>
        <div>{p.name}</div>
        <ProductActions product={p} columns={customColumns} onDone={onSaved} />
      </TD>
      <TD>
        <Input className="min-w-[140px]" value={variantName} onChange={(e) => setVariantName(e.target.value)} />
        <select className="mt-1 h-8 min-w-[140px] rounded-md border bg-background px-2 text-xs" value={variantActive ? "on" : "off"} onChange={(e) => setVariantActive(e.target.value === "on")}>
      ...
      <TD>
        <div className="grid min-w-[430px] grid-cols-4 gap-1">
          <Input inputMode="decimal" placeholder="gramas" value={filamentGrams} onChange={(e) => setFilamentGrams(normalizeDecimal(e.target.value))} />
          <Input inputMode="decimal" placeholder="horas" value={printTimeHours} onChange={(e) => setPrintTimeHours(normalizeDecimal(e.target.value))} />
          <Input inputMode="decimal" placeholder="W padrão" value={printerPowerWatts} onChange={(e) => setPrinterPowerWatts(normalizeDecimal(e.target.value))} />
          <Input inputMode="decimal" placeholder="emb. padrão" value={pack} onChange={(e) => setPack(normalizeDecimal(e.target.value))} />
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {filamentGrams || "0"}g · {printTimeHours || "0"}h · {String(effectivePower)}W · Fil. {formatBRL(cost.filamentCost)} · Energia {formatBRL(cost.energyCost)} · Emb. {formatBRL(cost.packagingCost)} · <span className="font-medium text-foreground">Total {formatBRL(cost.productionCost)}</span>
        </div>
        {missingProductionData ? <div className="mt-1 text-xs text-amber-600">Complete gramas, horas e potência para o custo de produção ficar completo.</div> : null}
      </TD>
      <TD>{st?.quantity ?? 0}</TD>
      <TD>{formatBRL(st?.revenue ?? "0")}</TD>
      ...
      <TD>
        <div className="flex min-w-[230px] gap-1">
          <Input className="w-20" type="number" min="0" placeholder="mín." value={min} onChange={(e) => setMin(e.target.value)} />
          <Input className="w-20" type="number" min="0" placeholder="ideal" value={ideal} onChange={(e) => setIdeal(e.target.value)} />
          <Button size="sm" variant="outline" onClick={() => void save()}>Salvar</Button>
        </div>
      </TD>
