...
export default function Resumo() {
  ...
  function csv() { const text = "Indicador,Valor\n" + rows.map(r => `${r[0]},${r[1]}`).join("\n") + `\nPedidos,${summary.orders_count}`; const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([text], { type: "text/csv;charset=utf-8" })); a.download = `resumo-${month}.csv`; a.click(); }
  function pdf() { const doc = new jsPDF(); doc.setFontSize(18); doc.text(`Freo Figures - Resumo ${month}`, 14, 20); doc.setFontSize(11); rows.forEach((r, i) => doc.text(`${r[0]}: ${formatBRL(r[1])}`, 14, 35 + i * 8)); doc.text(`Pedidos: ${summary.orders_count}`, 14, 35 + rows.length * 8); doc.save(`resumo-${month}.pdf`); }
