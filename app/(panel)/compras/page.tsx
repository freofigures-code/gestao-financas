...
type PluggyBill = {
  ...
  expected_settlement_amount: string | number | null;
  settlement_status: "future" | "paid" | "partial" | "unpaid" | "awaiting_confirmation";
...
function cardPaymentStatusLabel(row: PluggyTransaction) {
  if (row.account_type !== "CREDIT" || !isCreditCharge(row)) return "—";
  if (row.card_payment_status === "paid") return "Pago";
  if (row.card_payment_status === "partial") return "Pagamento parcial";
  if (row.card_payment_status === "unpaid") return "Fatura a pagar";
  if (row.card_payment_status === "future") return "Pagamento futuro";
  return "Aguardando confirmação da fatura";
}

function billStatusLabel(status: PluggyBill["settlement_status"]) {
  if (status === "paid") return "Paga";
  if (status === "partial") return "Pagamento parcial";
  if (status === "unpaid") return "Em aberto";
  if (status === "future") return "Vencimento futuro";
  return "Aguardando confirmação Pluggy";
}
...
export default function Compras() {
  ...
  const [bills, setBills] = useState<PluggyBill[]>([]);
  ...
  const loadPluggy = useCallback(async () => {
    ...
    const [accountResult, transactionResult, rulesResult, billsResult] = await Promise.all([
      supabase
      ...
      supabase
        ...
        .select("pluggy_bill_id,pluggy_account_id,due_date,bill_closing_date,total_amount,minimum_payment_amount,payments_total,expected_settlement_amount,settlement_status,settled_at")
        .gte("due_date", monthStart(month))
        .lt("due_date", nextMonthStart(month))
  ...
  const categorizationRows = useMemo(
    () => classifiableRows.filter((row) => row.classification !== "card_payment"),
    [classifiableRows],
  );
  const cardPurchaseRows = useMemo(
    () => transactions.filter((row) => isCreditCharge(row)),
    [transactions],
  );
  ...
  const receiverGroupMap = useMemo(() => new Map(receiverGroups.map((group) => [group.key, group])), [receiverGroups]);

  const cardPaidTotal = useMemo(() => cardPurchaseRows.filter((row) => row.card_payment_status === "paid").reduce((sum, row) => sum.plus(row.amount), new Decimal(0)), [cardPurchaseRows]);
  const cardFutureTotal = useMemo(() => cardPurchaseRows.filter((row) => row.card_payment_status === "future").reduce((sum, row) => sum.plus(row.amount), new Decimal(0)), [cardPurchaseRows]);
  const cardOpenTotal = useMemo(() => cardPurchaseRows.filter((row) => ["partial","unpaid","awaiting_confirmation"].includes(String(row.card_payment_status))).reduce((sum, row) => sum.plus(row.amount), new Decimal(0)), [cardPurchaseRows]);
    ...
    try {
      ...
      const body = await response.json().catch(() => ({})) as {
        ...
        sync?: {
          ...
          paidBills?: unknown;
          unsettledBills?: unknown;
          billSyncErrors?: unknown;
        };
      };
      ...
      const warningCodes = Array.isArray(body.sync?.creditCardWarningCodes)
        ...
        : [];
      const billCount = typeof body.sync?.creditCardBills === "number" ? body.sync.creditCardBills : 0;
      const billErrors = Array.isArray(body.sync?.billSyncErrors)
        ? body.sync!.billSyncErrors!.filter((value): value is string => typeof value === "string")
        : [];
      ...
      if (cards > 0 && cardTransactions === 0) {
        ...
        toast.warning(`Compras sincronizadas, mas a conciliação de faturas não pôde ser atualizada: ${billErrors[0]}`);
      } else {
        toast.success(`Pluggy atualizada: ${count} transação(ões), ${cards} cartão(ões), ${cardPurchases} compra(s) e ${billCount} fatura(s).`);
      }
    } catch (error) {
      ...
      {pluggyConnected ? (
        <>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <MetricCard label="Compras no cartão" value={pluggyLoading ? "Carregando..." : formatBRL(creditCardTotal.toFixed(2))} />
            <MetricCard label="Cartão já pago" value={pluggyLoading ? "Carregando..." : formatBRL(cardPaidTotal.toFixed(2))} />
            <MetricCard label="Cartão · pagamento futuro" value={pluggyLoading ? "Carregando..." : formatBRL(cardFutureTotal.toFixed(2))} />
            <MetricCard label="Cartão · em aberto/confirmação" value={pluggyLoading ? "Carregando..." : formatBRL(cardOpenTotal.toFixed(2))} />
          ...
          <Card>
            <CardHeader>
              <CardTitle>Compras individuais no cartão de crédito</CardTitle>
              <p className="text-sm text-muted-foreground">
                Cada linha abaixo é uma transação individual da conta CREDIT. O status de pagamento usa billId + Bills da Pluggy; nunca compara somente valor/data do débito “Pagamento de fatura”.
              </p>
            </CardHeader>
            <CardContent className="overflow-x-auto p-0">
              <Table>
                <THead>
                  <TR><TH>Data</TH><TH>Estabelecimento</TH><TH>Parcela</TH><TH>Fatura</TH><TH>Pagamento</TH><TH>Valor</TH><TH>Categoria / regra</TH></TR>
                </THead>
                <TBody>
                  {pluggyLoading ? <TR><TD colSpan={7}>Carregando compras do cartão...</TD></TR> : cardPurchaseRows.length === 0 ? (
                    <TR><TD colSpan={7} className="text-muted-foreground">Nenhuma compra individual de cartão foi retornada neste mês.</TD></TR>
                  ) : cardPurchaseRows.map((row) => {
                    const key = persistedOrFallbackMatchKey(row);
                    const group = receiverGroupMap.get(key);
                    return (
                      <TR key={`card-${row.id}`}>
                        ...
                        <TD>
                          ...
                          {row.total_amount ? <div className="text-xs text-muted-foreground">Compra total: {formatBRL(String(row.total_amount))}</div> : null}
                        </TD>
                        <TD>
                          {row.bill_id ? (
                            <>
                              <div className="font-medium">{row.bill_due_date ? `Vence ${formatDate(row.bill_due_date)}` : "Fatura identificada"}</div>
                              <div className="text-xs text-muted-foreground">ID {row.bill_id.slice(0, 8)}…</div>
                            </>
                          ) : (
                            <div>{row.bill_forecast_date ? `Prevista ${row.bill_forecast_date}` : "Ainda sem fatura fechada"}</div>
                          )}
                        </TD>
                        <TD>
                          <div className="font-medium">{cardPaymentStatusLabel(row)}</div>
                          {row.card_payment_status === "future" ? <div className="text-xs text-muted-foreground">Não saiu do caixa ainda.</div> : null}
                        </TD>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Faturas conciliadas pela Pluggy</CardTitle>
              <p className="text-sm text-muted-foreground">
                “Paga” só aparece quando a regra oficial de quitação da Pluggy pode ser confirmada pelos ciclos de Bills. Sem evidência suficiente, o sistema mantém “Aguardando confirmação” em vez de adivinhar.
              </p>
            </CardHeader>
            <CardContent className="overflow-x-auto p-0">
              <Table>
                <THead><TR><TH>Vencimento</TH><TH>Total da fatura</TH><TH>Pagamento registrado</TH><TH>Valor de conciliação</TH><TH>Status</TH></TR></THead>
                <TBody>
                  {pluggyLoading ? <TR><TD colSpan={5}>Carregando faturas...</TD></TR> : bills.length === 0 ? (
                    <TR><TD colSpan={5} className="text-muted-foreground">Nenhuma entidade Bill com vencimento neste mês foi copiada da Pluggy.</TD></TR>
                  ) : bills.map((bill) => (
                    <TR key={bill.pluggy_bill_id}>
                      <TD>{formatDate(bill.due_date)}</TD>
                      <TD className="font-medium">{formatBRL(String(bill.total_amount))}</TD>
                      <TD>{formatBRL(String(bill.payments_total))}</TD>
                      <TD>{bill.expected_settlement_amount === null ? "—" : formatBRL(String(bill.expected_settlement_amount))}</TD>
                      <TD>
                        <div className="font-medium">{billStatusLabel(bill.settlement_status)}</div>
                        {bill.settled_at ? <div className="text-xs text-muted-foreground">Confirmado em {new Date(bill.settled_at).toLocaleDateString("pt-BR", { timeZone: "America/Sao_Paulo" })}</div> : null}
                      </TD>
                    </TR>
                  ))}
                </TBody>
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            ...
            <Card>
              ...
              <CardContent className="space-y-2 text-sm text-muted-foreground">
                ...
                <p><b className="text-foreground">Pagamento da fatura:</b> é detectado automaticamente na conta BANK e sai do caixa. Ele não recebe categoria de compra. Os itens daquela fatura só ficam “Pagos” quando o billId e a quitação da entidade Bill forem confirmados pela Pluggy.</p>
                ...
                <TBody>
                  {pluggyLoading ? <TR><TD colSpan={8}>Carregando...</TD></TR> : classifiableRows.length === 0 ? (
                    ...
                    <TR key={row.id}>
                      ...
                      <TD>
                        {row.classification === "card_payment" ? (
                          <span className="text-sm text-muted-foreground">Conciliação automática de fatura</span>
                        ) : (
                          <ClassificationSelect
